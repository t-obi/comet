...
theorem exercise3:
  assumes 1: "A \<^bold>\<rightarrow> B"
  assumes 2: "B \<^bold>\<rightarrow> C"
  shows "A \<^bold>\<rightarrow> C"
proof -
  have 3: "((B \<^bold>\<rightarrow> C) \<^bold>\<rightarrow> A \<^bold>\<rightarrow> (B \<^bold>\<rightarrow> C))" by (rule A2[where A = "B \<^bold>\<rightarrow> C" and B = "A"])
  from 3 2 have 4: "A \<^bold>\<rightarrow> (B \<^bold>\<rightarrow> C)" by (rule ModusPonens)
  have 5: "(A \<^bold>\<rightarrow> (B \<^bold>\<rightarrow> C)) \<^bold>\<rightarrow> ((A \<^bold>\<rightarrow> B) \<^bold>\<rightarrow> (A \<^bold>\<rightarrow> C))" by (rule A3)
  from 5 4 have 6:" ((A \<^bold>\<rightarrow> B) \<^bold>\<rightarrow> (A \<^bold>\<rightarrow> C))" by (rule ModusPonens)
  from 6 1 have "A \<^bold>\<rightarrow> C" by (rule ModusPonens)
  thus ?thesis.
qed
